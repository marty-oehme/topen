...
from topen import TConf, _IO, _cmd_clean, _is_cleanable, _resolve_task
...
class TestIsCleanable:
    def test_pending_task_is_not_cleanable(self):
        task = Mock()
        task.__getitem__ = Mock(side_effect=lambda k: {"status": "pending"}[k])
        assert _is_cleanable(task) is False

    def test_recurring_task_is_not_cleanable(self):
        task = Mock()
        task.__getitem__ = Mock(side_effect=lambda k: {"status": "recurring"}[k])
        assert _is_cleanable(task) is False

    def test_completed_task_is_cleanable(self):
        task = Mock()
        task.__getitem__ = Mock(side_effect=lambda k: {"status": "completed"}[k])
        assert _is_cleanable(task) is True

    def test_deleted_task_is_cleanable(self):
        task = Mock()
        task.__getitem__ = Mock(side_effect=lambda k: {"status": "deleted"}[k])
        assert _is_cleanable(task) is True

    def test_missing_task_is_cleanable(self):
        assert _is_cleanable(None) is True
